...
to (via `getdata`, `create_scratch!`, `get_scratch`).
...
Base.empty!(s::DataSeries{:output}) = empty_output!(s.backend, s.sim, s.name)
...
Base.empty!(s::DataSeries{:scratch}) = empty_scratch!(s.backend, s.sim, s.name)
