...
using SimulationBasedInference: InMemoryStorage, InMemoryStorageHandle, DiskStorageBackend,
...
@testset "SimulationDataSet (on disk, JLD2)" begin
    mktempdir() do dir
        backend = DiskStorageBackend(format"JLD2", dir)
        dataset = SimulationDataSet(backend)
        @test length(dataset) == 0
        ...
        backend = DiskStorageBackend(format"JLD2", dir)
        reopened = SimulationDataSet(backend)
...
@testset "StorageHandle - InMemory" begin
    ...
    @test handle.index == i1
    
    # Test that we can access data through the handle
    @test SimulationBasedInference.getinputs(handle) == [1.0, 2.0]
    @test SimulationBasedInference.getmetadata(handle)[:iter] == 1
    
    # Test output operations with handle (fast path - no auto-open/close)
    store_output!(handle, :y, [0.5])
    store_output!(handle, :y, [0.6])
    @test get_outputs(handle, :y) == [[0.5], [0.6]]
    @test output_length(handle, :y) == 2
    @test has_output(handle, :y)
    
    # Test scratch operations with handle (handle-specific storage)
    store_scratch!(handle, :temp, "data1")
    store_scratch!(handle, :temp, "data2")
    @test get_scratch(handle, :temp, 1) == "data1"
    @test get_scratch(handle, :temp, 2) == "data2"
    @test scratch_length(handle, :temp) == 2
    @test has_scratch(handle, :temp)
    @test :temp in scratch_names(handle)
    ...
    store_scratch!(handle2, :temp2, "data3")
    @test has_scratch(handle2, :temp2)  # Scratch exists for this sim
    ...
    @test has_scratch(handle3, :temp) == false  # Cleared on close
    
    # But backend data should persist
    @test SimulationBasedInference.getinputs(handle3) == [1.0, 2.0]
    @test get_outputs(handle3, :y) == [[0.5], [0.6]]
...
@testset "StorageHandle - JLD2" begin
    mktempdir() do dir
        dataset = SimulationDataSet(DiskStorageBackend(format"JLD2", dir))
        ...
        store_scratch!(handle, :scratch_data, "temp_value")
        @test get_scratch(handle, :scratch_data, 1) == "temp_value"
        ...
        @test SimulationBasedInference.getinputs(handle2) == [1.0, 2.0]
        @test get_outputs(handle2, :y) == [[0.5], [0.6]]
        @test has_scratch(handle2, :scratch_data) == false  # Cleared on close
        ...
        @test SimulationBasedInference.getinputs(handle4) == [1.0, 2.0]
        @test get_outputs(handle4, :y) == [[0.5], [0.6]]
        close(handle4)
        
        handle5 = open(dataset.backend, 2)
        @test SimulationBasedInference.getinputs(handle5) == [5.0, 6.0]
        @test get_outputs(handle5, :z) == [[1.0]]
        close(handle5)
    end
end
...
@testset "Handle Resource Safety - finalize" begin
    ...
    store_scratch!(handle, :temp, "should_be_cleared")
    ...
    @test has_scratch(handle2, :temp) == false
    close(handle2)
end
...
@testset "Handle Scratch Isolation" begin      
    ...
    store_scratch!(handle1, :shared, "from_handle1")
    
    i2 = SimulationBasedInference.allocate!(backend, [2.0])
    handle2 = open(backend, i2)
    store_scratch!(handle2, :shared, "from_handle2")
    
    # Each handle sees only its own scratch
    @test get_scratch(handle1, :shared, 1) == "from_handle1"
    @test get_scratch(handle2, :shared, 1) == "from_handle2"
    ...
    handle3 = open(backend, i1)
    @test has_scratch(handle3, :shared) == false  # Cleared when handle1 closed
