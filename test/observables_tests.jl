...
@testset "TimeSampled" begin
    ...
    # TimeSampled uses handle-local scratch, so the SimulationData must wrap an open handle
    function with_handle_data(f)
        handle = open(SBI.InMemoryStorage())
        i = allocate!(handle, Float64[])
        try
            f(SimulationData(handle, i))
        finally
            close(handle)
        end
    end
    # case 1: scalar state
    with_handle_data() do data
        state = (x = 0.0,)
