...
N_obs = 100
σ_true = 10.0
data = generate_synthetic_dataset(N_obs, σ_true, p_true)

# plot the data
let fig = Makie.Figure(),
    ax = Makie.Axis(fig[1,1], xticks=(1:30:length(data.ts), Dates.format.(data.ts[1:30:end], "YYYY-mm")));
    ax.xticklabelrotation = π/4
    lines = Makie.lines!(ax, 1:length(data.ts), data.y_true[:,1], linewidth=2.0)
    points = Makie.scatter!(ax, data.idx, data.y_obs, color=:black)
...
eks_sol = @time solve(inference_prob, EKS(), EnsembleThreads(), n_ens=128, verbose=false, rng=rng);
eks_res = summarize_ensemble(eks_sol)

# Solve with ESMDA
esmda_sol = @time solve(inference_prob, ESMDA(maxiters=10), EnsembleThreads(), n_ens=128, verbose=false, rng=rng);
...
let fig = Makie.Figure(size=(800,600)),
    ...
    d1, _ = plot_density!(ax, eks_res, 2, :blue, offset=3.0)
    d2, _ = plot_density!(ax, esmda_res, 2, :orange, offset=2.0)
    d3, _ = plot_density!(ax, snpe_res, 2, :green, offset=1.0)
...
let fig = Makie.Figure(size=(900,600)),
    ...
    plt3 = plot_predictions!(ax, hmc_res, data.ts, :green)
